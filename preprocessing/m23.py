...
def process_musicxml_and_infer_chords(input_filename, output_dir: str):
    ...
    for quantized_seq in quantized_sequences:
        ...
        except chord_inference.EmptySequenceError:
            # print(f"EmptySequence error: {e}")
            continue
        except chord_inference.UncommonTimeSignatureError:
            # print(f"UncommonTimeSignatureError: {e}")
            continue
        except chord_inference.SequenceAlreadyHasChordsError:
            # print(f"AlreadyHasChordError: {e}")
            continue
    ...
    for i, chorded_seq in enumerate(concatenated_sequences):
        try:
            if chorded_seq.text_annotations[0].annotation_type == CHORD_SYMBOL:
                chord_symbol = chorded_seq.text_annotations[0].text
                path = Path(output_dir)
                fp = path / chord_symbol
...
if __name__ == "__main__":
    ...
    ## Process
    from all_files import ALL_FILES as all_mxl
